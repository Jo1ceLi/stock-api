...
class AccountService {
    static async findAccount(req: Request, res: Response, next: NextFunction) {
        ...
        const {
            id,
        } = jwt.decode(authentication) as decode;
        const user = await User.findOne(id);
        if (!user) { return next(new ApiExcption(403, 'Forbidden')); }
        if (!user.tdAccountId.includes(accountId)) { return next(new ApiExcption(403, 'Forbidden')); }
